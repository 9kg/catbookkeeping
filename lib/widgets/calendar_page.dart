...
class CalendarMonthDialog extends StatefulWidget {
  const CalendarMonthDialog(
      {Key key, this.nowYear: -1, this.nowMonth: -1, this.minYear: 1971, this.maxYear: 2055, this.checkTap})
      : super(key: key);
...
class _CalendarMonthDialogState extends State<CalendarMonthDialog> with SingleTickerProviderStateMixin {
  int _selectedYear;
  ...
  Widget _buildTitle() {
    return Container(
      ...
      child: Stack(
        alignment: Alignment.center,
        children: <Widget>[
          Text('$_selectedYear-${_selectedMonth.toString().padLeft(2, '0')}', style: TextStyle(fontSize: 16)),
          Positioned(
            left: 20,
            child: HighLightWell(
              child: SizedBox(
              ...
              onTap: () {
                if (_selectedYear <= widget.minYear) {
                  Utils.show('${widget.minYear} ${Strings.minYear}');
                } else {
                  setState(() {
                    _selectedYear -= 1;
                  });
            ...
            child: HighLightWell(
              ...
              onTap: () {
                if (_selectedYear >= widget.maxYear) {
                  Utils.show('${widget.maxYear} ${Strings.maxYear}');
                } else {
                  setState(() {
                    _selectedYear += 1;
                  });
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: index + 1 == _selectedMonth ? Colours.app_main : Colors.white,
                    borderRadius: BorderRadius.circular(itemWidth / 2),
                  ),
                  child: Text(
                    '${index + 1}月',
                    style: TextStyle(fontSize: 16, color: index + 1 == _selectedMonth ? Colors.white : Colors.black),
            ...
            children: <Widget>[
              ...
              Expanded(
                ...
                child: HighLightWell(
                  ...
                  onTap: () {
                    widget.checkTap('$_selectedYear', '${_selectedMonth.toString().padLeft(2, '0')}');
                    NavigatorUtils.goBack(context);
                  },
                ),
              )
