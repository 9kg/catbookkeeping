...
class BillSearchListState extends State<BillSearchList> {
  ...
  // 初始化数据
  Future _initDataList() async {
    // 时间戳
    int startTime = DateTime(int.parse(widget.year), int.parse(widget.month), 1, 0, 0, 0, 0).millisecondsSinceEpoch;
    int endTime = DateTime(int.parse(widget.year), int.parse(widget.month),
            Utils.getDaysNum(int.parse(widget.year), int.parse(widget.month)), 23, 59, 59, 999)
        .millisecondsSinceEpoch;
    dbHelp.getBillList(startTime, endTime, categoryName: widget.searchCategoryName).then((models) {
      DateTime _preTime;
      ...
      void buildGroup() {
        ...
        DateTime time = DateTime.fromMillisecondsSinceEpoch(itemList.first.updateTimestamp);
        String groupDate =
            '${time.year}-${time.month.toString().padLeft(2, '0')}-${time.day.toString().padLeft(2, '0')}';
        BillRecordGroup group = BillRecordGroup(groupDate, expenMoney, incomeMoney);
      ...
      List.generate(length, (index) {
        ...
        if (_preTime == null) {
          ...
          // 存在两条或以上数
          DateTime time = DateTime.fromMillisecondsSinceEpoch(item.updateTimestamp);
          //判断账单是不是在同一天
          if (time.year == _preTime.year && time.month == _preTime.month && time.day == _preTime.day) {
            //如果是同一天
            addAction(item);
            if (index == length - 1) {
              //这是最后一条数据
      ...
      appBar: MyAppBar(
        titleWidget: Column(
          ...
          children: <Widget>[
            ...
            Text(
              '${widget.year}-${widget.month}-01-${widget.year}-${widget.month}-${Utils.getDaysNum(int.parse(widget.year), int.parse(widget.month)).toString()}',
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Row(
                      children: <Widget>[
                        Image.asset(
                          Utils.getImagePath('${model.image}'),
  ...
  Widget _buildTimeTag(BillRecordGroup group) {
    ...
    if (group.totalRevenue > 0) {
      moneyString = moneyString +
          ' ${Strings.revenue} ${Utils.formatDouble(double.parse(group.totalRevenue.toStringAsFixed(2)))}';
    }
    if (group.totalExpenses > 0) {
      moneyString = moneyString +
          '${group.totalRevenue > 0 == true ? '  ' : ''} ${Strings.expenses} ${Utils.formatDouble(double.parse(group.totalExpenses.toStringAsFixed(2)))}';
    }
  ...
  void _showBottomSheet(BillRecordModel model) {
    if (model == null) {
      Utils.show('查询错误');
      return;
    }
